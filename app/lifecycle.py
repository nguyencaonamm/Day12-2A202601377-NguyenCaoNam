...
class Lifecycle:
    ...
    def request_shutdown(self, signum=None, frame=None) -> None:
        ...
        # Bật cờ để chặn các request mới
        self.shutting_down = True
        
        # Gọi lại handler cũ nếu có (Quan trọng để server có thể thực sự tắt)
    ...
    def install(self) -> None:
        ...
        # SIGTERM (Cloud yêu cầu tắt), SIGINT (Bấm Ctrl+C)
        for sig in (signal.SIGTERM, signal.SIGINT):
            # Lưu lại handler mặc định của uvicorn
            self._previous[sig] = signal.getsignal(sig)
            # Thay bằng handler request_shutdown của chúng ta
